...
def init_db():
    ...
    # Create Authorized Data Table
    c.execute('''CREATE TABLE IF NOT EXISTS authorized_data
                 (student_id TEXT PRIMARY KEY,
                  name TEXT,
                  face_encoding BLOB)''')
    
    # Create Webcam Captured Data Table
    ...
    # Create Unauthorized Attempts Table
...
def is_authorized(face_encoding):
    """Check if a face encoding matches any authorized person"""
    conn = sqlite3.connect(Config.DB_PATH)
    c = conn.cursor()
    ...
    if ret:
        ...
        if face_encoding is not None:
            ...
            if student_id:
                ...
                # Save frame for security notification
...
def security_verify(attempt_id, decision):
    """Handle security verification from email links"""
    try:
        conn = sqlite3.connect(Config.DB_PATH)
        c = conn.cursor()
...
def manage_authorized():
    """View for managing authorized persons"""
    conn = sqlite3.connect(Config.DB_PATH)
    c = conn.cursor()
...
def edit_authorized(student_id):
    """Edit an authorized person's details"""
    if request.method == 'POST':
        name = request.form['name']
        try:
            conn = sqlite3.connect(Config.DB_PATH)
